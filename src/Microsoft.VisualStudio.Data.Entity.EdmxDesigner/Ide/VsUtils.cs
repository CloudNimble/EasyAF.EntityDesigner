...
namespace Microsoft.VisualStudio.Data.Entity.EdmxDesigner.Ide
{
    ...
    internal static class VsUtils
    {
            ...
            string message, __ACTIVITYLOG_ENTRYTYPE entryType = __ACTIVITYLOG_ENTRYTYPE.ALE_INFORMATION)
        {
            ...
            try
            {
                if (PackageManager.Package?.GetService(typeof(SVsActivityLog)) is IVsActivityLog activityLog)
                {
        ...
        internal static Func<string, object> GetLocalApplicationSettingImplementation(Project project)
        {
            ...
            return appSettingName =>
            {
                try
                {
                    ...
                    {
                        var configurationFile = GetProjectConfigurationFile(project, PackageManager.Package);
        ...
        internal static string ResolveMacro(Project project, string macroName, Dictionary<string, string> customMacros)
        {
            ...
            if (project != null)
            {
                var hierarchy = GetVsHierarchy(project, PackageManager.Package);
        ...
        internal static string GetProjectPathWithName(Project project, out bool hasFileName)
        {
            ...
            if (project != null)
            {
                var projectDirectoryInfo = GetProjectRoot(project, PackageManager.Package);
        ...
        internal static string GetFileContentsFromBufferOrDisk(string filePath)
        {
            ...
            var docData = VSHelpers.GetDocData(PackageManager.Package, filePath);
        ...
        internal static bool TryGetRelativePathInProject(Project project, string path, out string relativePath)
        {
            ...
            var projectDirectory = GetProjectRoot(project, PackageManager.Package);
        ...
        {
            ILocalRegistry3 localRegistry = PackageManager.Package.GetService(typeof(ILocalRegistry)) as ILocalRegistry3;
        ...
        internal static void WriteCheckoutTextFilesInProject(IDictionary<string, object> filesMap)
        {
            ...
            WriteCheckoutFilesInProject(
                PackageManager.Package, filesMap, fileDataObject => fileDataObject as string,
            ...
            PersistObjectDataDelegate persistObjectDataCallback)
        {
            ...
            foreach (var filePath in filesToQueryEdit.Keys)
            {
                ...
                finally
                {
                    ...
                    {
                        UnlockRunningDocument(PackageManager.Package, docCookie, _VSRDTFLAGS.RDT_EditLock);
                    }
                }
            }
                    ...
                    switch (GetLanguageForProject(sourceProjectItem.ContainingProject))
                    {
                        case LangEnum.CSharp:
                            return GetProjectItemForDocument(
                                Path.ChangeExtension(sourceFileName, FileExtensions.CsExt), PackageManager.Package);

                        case LangEnum.VisualBasic:
                            return GetProjectItemForDocument(
                                Path.ChangeExtension(sourceFileName, FileExtensions.VbExt), PackageManager.Package);
                    }
                }
            }
        ...
        internal static void RunCustomTool(ProjectItem projectItem)
        {
            ...
            if (generatedCodeItem == null)
            {
                ...
                else
                {
                    ...
                    {
                        canEdit = VSHelpers.CheckOutFilesIfEditable(PackageManager.Package, new[] { projectFullPath });
                    }
                }
            }
            else
            {
                canEdit = VSHelpers.CheckOutFilesIfEditable(PackageManager.Package, new[] { generatedCodeItem.get_FileNames(1) });
            }
        ...
        internal static string ConstructTargetPathForDatabaseFile(Project project, ProjectItems targetCollection, string fileName)
        {
            ...
            var projectDir = GetProjectRoot(project, PackageManager.Package);
            if (null != projectDir)
            {
                if (null == targetPath)
                {
        ...
        public static void EnsureProvider(EFArtifact artifact)
        {
            ...
            if (artifact.StorageModel() != null)
            {
                ...
                EnsureProvider(providerInvariantName, false, project, PackageManager.Package);
            }
        }
