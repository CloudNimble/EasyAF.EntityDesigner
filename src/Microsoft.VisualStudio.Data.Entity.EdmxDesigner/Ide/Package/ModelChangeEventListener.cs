...
namespace Microsoft.VisualStudio.Data.Entity.EdmxDesigner.Ide.Package
{
                                              ...
                                              IVsSolutionEvents, IDisposable
    {
        ...
        {
            IVsRunningDocumentTable rdt = PackageManager.Package.GetService(typeof(SVsRunningDocumentTable)) as IVsRunningDocumentTable;
            rdt?.AdviseRunningDocTableEvents(this, out trackRDTEventsCookie);
        }

        private void StopTrackingRDTEvents()
        {
            if (PackageManager.Package.GetService(typeof(SVsRunningDocumentTable)) is IVsRunningDocumentTable rdt)
            {
        ...
        {
            IVsTrackProjectDocuments2 trackProjDocs = PackageManager.Package.GetService(typeof(SVsTrackProjectDocuments)) as IVsTrackProjectDocuments2;
            trackProjDocs?.AdviseTrackProjectDocumentsEvents(this, out trackDocEventsCookie);
        }

        private void StopTrackingProjectEvents()
        {
            if (PackageManager.Package.GetService(typeof(SVsTrackProjectDocuments)) is IVsTrackProjectDocuments2 trackProjDocs)
            {
        ...
        {
            IVsSolution trackSol = PackageManager.Package.GetService(typeof(SVsSolution)) as IVsSolution;
            trackSol?.AdviseSolutionEvents(this, out trackSolEventsCookie);
        }

        private void StopTrackingSolutionEvents()
        {
            if (PackageManager.Package.GetService(typeof(SVsSolution)) is IVsSolution trackSol)
            {
