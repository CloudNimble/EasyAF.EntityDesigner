...
namespace Microsoft.VisualStudio.Data.Entity.EdmxDesigner.Ide.ModelWizard
{
    ...
    public class ModelObjectItemWizard : IWizard
    {
        ...
        private static string GetFolderNameForNewItems(DTE2 dte, Project activeProject)
        {
            ...
            {
                var di = VsUtils.GetProjectRoot(selectedItem.Project, PackageManager.Package);
                path = di.FullName;
            }
            ...
            {
                var di = VsUtils.GetProjectRoot(activeProject, PackageManager.Package);
                path = di.FullName;
            }
            ...
            if (VsUtils.IsWebSiteProject(activeProject))
            {
                DirectoryInfo di = new DirectoryInfo(
                    Path.Combine(
                        VsUtils.GetProjectRoot(activeProject, PackageManager.Package).FullName,
