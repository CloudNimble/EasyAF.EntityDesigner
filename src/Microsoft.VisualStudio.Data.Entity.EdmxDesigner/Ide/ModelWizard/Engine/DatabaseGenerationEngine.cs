    ...
    internal class DatabaseGenerationEngine : DatabaseEngineBase
    {
        ...
        internal static bool UpdateEdmxAndEnvironment(ModelBuilderSettings settings)
        {
                ...
                && settings.DdlStringReader != null)
            {
                ...
                if (VsShellUtilities.IsDocumentOpen(
                    PackageManager.Package, canonicalFilePath, Guid.Empty, out IVsUIHierarchy hier, out uint itemId, out IVsWindowFrame frame) == false)
                {
                    VsShellUtilities.OpenDocument(PackageManager.Package, canonicalFilePath);
                }
            }
...
#endif
            var ddlProjectItem = VsUtils.GetProjectItemForDocument(ddlFileName, PackageManager.Package);
        ...
        internal static FileInfo ResolveAndValidatePath(Project project, string path, PathValidationErrorMessages errorMessages)
        {
                ...
                == false)
            {
                ...
                if (VsUtils.GetProjectItemForDocument(resolvedPath, PackageManager.Package) == null)
                {
