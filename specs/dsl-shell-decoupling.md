...
| ~~`Diagram/DiagramImageHelper.cs`~~ | ~~`ThemeUtils` (GDI rasterization)~~ | **Done.** The tinting moved to the shell; the designer receives finished images — see below |
...
### Icons: the designer holds them, the shell tints them

`DiagramImageHelper` was the last real blocker, and the resolution was smaller than the theming plan assumed. The class does two things: it recolours icons with GDI, and it caches them for `EntityTypeShape` to draw. Only the first is host work.

So the class stays where it is and keeps the cache — `EntityTypeShape` is untouched — but `Load(Color)` became `SetPropertyIcons(...)` and `GetHeaderIcons` no longer builds anything, it only returns what was registered. `VsDiagramIcons` in the Package does the `ThemeUtils` work and pushes, alongside the palette, because the two belong together: property icons are tinted against the compartment fill, so applying a palette without refreshing icons leaves them keyed to the previous theme.

What made this safe to do without settling the theming design first: **the headless renderer never touches this class.** It emits its own SVG symbols through `SvgIconManager`. So an unsupplied icon set is not a degraded rendering, it is the correct state for a host that never paints — `GetHeaderIcons` returns null and the shape's image fields stay empty.

That leaves `unified-theming.md` item 2 free to decide what icons *look* like, which is the part with open design questions, rather than where the code lives.
...
All three hold. The designer now references only `Microsoft.Data.Tools.Design.XmlCore`, `Microsoft.VisualStudio.Data.Tools.Design.XmlCore` and `Microsoft.Data.Entity.Design.Model`, and no shell type appears anywhere in its source. The reference now runs the other way: `Microsoft.VisualStudio.Data.Entity.Design` references the Dsl, because the UI acts on the surface.

Two consequences of the reversal worth recording:

- The Dsl grew a direct `Microsoft.VisualStudio.TextTemplating` package reference. `GeneratedCode/DirectiveProcessor.cs` derives from `RequiresProvidesDirectiveProcessor` and had been getting it transitively through the reference that was deleted. A reference is not cut until the compiler agrees — grep cannot see transitive packages.
- `Tx_ReferentialContraint` moved into the Dsl's own `Resources.resx`, since `AssociationConnector` was reaching into the shell's resources for one undo label. The `.Designer.cs` accessor had to be added by hand: it is generated by a Visual Studio custom tool, not by the build, so editing the `.resx` alone compiles the string into the assembly but leaves no way to reference it.

**Still outstanding:** `Microsoft.VisualStudio.Data.Tools.Design.XmlCore` remains a Dsl reference. Nothing in the designer's own source uses its shell types any more, so this is now a question of whether the remaining foundation types it provides (`EditingContext` lives in the non-VS `XmlCore`) justify keeping it — a smaller, separate exercise.
